...
from financeblog import app, db, bcrypt, mail
from financeblog.models import User, Post
from financeblog.forms import (RegistrationForm, LoginForm, UpdateAccountForm, 
...
def send_reset_email(user):
    ...
    msg = Message('Password Reset Request', 
                    sender='couchmoneyonline@gmail.com', 
                    recipients=[user.email])
    msg.body = f'''Click the link below to reset your password
{url_for('reset_token', token=token, _external=True)}

If you did not request for a password reset then simply ignore this email
'''
    mail.send(msg)
...
def reset_token(token):
    ...
    if user is None:
        flash("Your reset token is expired or invalid", "warning")
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(
            'Your password has been changed successfully! Login to continue', 'success')
        return redirect(url_for('login'))
